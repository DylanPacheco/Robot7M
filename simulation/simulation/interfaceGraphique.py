...
from roogleTranslate import Translate
from API.strat70 import *
import random
import time, sys
...
def clavier(event):
    ...
    if touche =='1':
        robot.setStrat(Strat70())
        while not self.strategie.stop():
            self.strategie.update()
        print("Fini")
    
    if touche =='2':
        robot.setStrat(Strat(2, [7, 'R']))
        print(robot.strat.Lstrat)
        for i in robot.strat.Lstrat:
            robot.interprete_strat(i)
            fenetre.after(100,rafraichir,a1)
        print("Fini")
...
def rafraichir(arene):
    ...
    if(len(arene.liste_robot) == 1):
        ...
        print(arene.liste_robot[0].safficher())
